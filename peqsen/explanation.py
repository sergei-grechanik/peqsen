...
def transform_script(script, fun, cache=None):
    """Transform the given script by applying fun to its nodes in the bottom-up order. Sharing is
    preserved."""
    if cache is None:
        cache = {}
    ...
    if isinstance(script, RunAllScript):
        res = fun(RunAllScript(tuple(transform_script(s, fun, cache) for s in script.subscripts)))
    elif isinstance(script, IthElementScript):
        res = fun(IthElementScript(transform_script(script.script, fun, cache), script.index))
    elif isinstance(script, IncidentNode):
        res = fun(IncidentNode(transform_script(script.hyperedge, fun, cache), script.index))
    elif isinstance(script, FreeNodeScript):
        res = fun(script)
    elif isinstance(script, MatchScript):
        res = fun(MatchScript(elements=script.elements,
                              hyperedge_scripts={h: transform_script(s, fun, cache)
                                                 for h, s in script.hyperedge_scripts.items()},
                              merge_scripts=[(n1, n2, transform_script(s, fun, cache))
                                             for n1, n2, s in script.merge_scripts],
                              node_scripts={n: transform_script(s, fun, cache)
                                            for n, s in script.node_scripts.items()}))
    elif isinstance(script, RuleApplicationScript):
        res = fun(RuleApplicationScript(script.rule,
                                        transform_script(script.match_script, fun, cache)))
    elif isinstance(script, AddTermsScript):
        res = fun(script)
    else:
        raise ValueError("Don't know how to transform this script {}".format(script))

    cache[script] = res
    return res

def traverse_script(script, fun, cache=None):
    """Traverse the script in bottom-up order by applying the function fun to all its nodes."""
    if cache is None:
        cache = set()
    ...
    if isinstance(script, RunAllScript):
        for s in script.subscripts:
            traverse_script(s, fun, cache)
    elif isinstance(script, IthElementScript):
        traverse_script(script.script, fun, cache)
    elif isinstance(script, IncidentNode):
        traverse_script(script.hyperedge, fun, cache)
    elif isinstance(script, FreeNodeScript):
        pass
    elif isinstance(script, MatchScript):
        for h, s in script.hyperedge_scripts.items():
        ...
        traverse_script(script.match_script, fun, cache)
    elif isinstance(script, AddTermsScript):
        pass
    else:
        raise ValueError("Don't know how to traverse this script {}".format(script))

    fun(script)

def collect_script_nodes(script, predicate=None):
    res = []

    def _fun(script, predicate=predicate, res=res):
        if predicate is not None:
            if isinstance(predicate, type):
                if not isinstance(script, predicate):
                    return
            elif not predicate(script):
                return
        res.append(script)

    traverse_script(script, _fun)
    return res

def compose_scripts(first, second, term):
    """Compose two scripts so that the `term` in the second script is not simply added, but instead
    is generated by the first script"""

    if first is None:
        raise ValueError("first mustn't be none")

    def _fun(script, first=first, second=second, term=term):
        if isinstance(script, AddTermsScript) and term in script.terms:
            if len(script.terms) > 1:
                raise ValueError("Composing scripts for multi-term AddTermsScript is not supported")
            return first
        else:
            return script

    return transform_script(second, _fun)
...
class ExplanationTracker(Listener):
    ...
    def script(self, requirement):
        cache = {}
        r = requirement
        if isinstance(r, Hyperedge):
            return self.script_for_hyperedge(r, cache)
        elif isinstance(r, tuple):
            return self.script_for_merge(r[0], r[1], cache)
        elif isinstance(r, dict):
            return self.script_for_match(r, cache)
        elif isinstance(r, list):
            scripts = []
            for sub_r in r:
                scripts.append(self.script(sub_r))
